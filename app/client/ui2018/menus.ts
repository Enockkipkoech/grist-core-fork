...
export function closeRegisteredMenu() {
  if (_lastOpenedController) {
    _lastOpenedController.close();
  }
}
...
export function menu(
  createFunc: weasel.MenuCreateFunc,
  options?: weasel.IMenuOptions
): DomElementMethod {
...
export const cssMenuElem = styled(
  "div",
  `
...
export function linkSelect<T>(
  ...
  const elem = select(obs, optionArray, {
    buttonCssClass: _btn.className,
    ...options,
  });
...
export function multiSelect<T>(
  selectedOptions: MutableObsArray<T>,
  availableOptions: MaybeObsArray<IOption<T>>,
  options: IMultiSelectUserOptions = {},
  ...domArgs: DomElementArg[]
) {
  const selectedOptionsSet = Computed.create(
    null,
    selectedOptions,
    (_use, opts) => new Set(opts)
  );

  const selectedOptionsText = Computed.create(
    null,
    selectedOptionsSet,
    (use, selectedOpts) => {
      ...
      const optionArray = Array.isArray(availableOptions)
        ? availableOptions
        : use(availableOptions);
      return optionArray
  ...
  function buildMultiSelectMenu(ctl: weasel.IOpenController) {
    return cssMultiSelectMenu(
      ...
      (elem) => {
        ...
        style.minWidth =
          ctl.getTriggerElem().getBoundingClientRect().width + "px";
      ...
      dom.domComputed(selectedOptionsSet, (selectedOpts) => {
        return dom.forEach(availableOptions, (option) => {
          ...
          return cssCheckboxLabel(
            ...
            cssCheckboxText(
              fullOption.label,
              testId("multi-select-menu-option-text")
            ),
  ...
  return cssSelectBtn(
    ...
    cssMultiSelectSummary(
      ...
      cssMultiSelectSummary.cls(
        "-placeholder",
        (use) => use(selectedOptionsSet).size === 0
      )
    ),
    icon("Dropdown"),
    (elem) => {
      weasel.setPopupToCreateDom(
        elem,
        (ctl) => buildMultiSelectMenu(ctl),
...
export function formSelect(
  ...
  optionArray: MaybeObsArray<IOption<string>>,
  ...
  const container: Element = cssSelectBtnContainer(
    dom(
      "select",
      {
        class: cssSelectBtn.className,
        style: "height: 42px; padding: 12px 30px 12px 12px;",
      },
      dom.prop("value", obs),
      dom.on("change", (_, elem) => {
        obs.set(elem.value);
      }),
...
export function inputMenu(
  ...
  function inputTrigger(triggerElem: Element, ctl: weasel.PopupControl): void {
    dom.onElem(triggerElem, "input", () => {
      (triggerElem as HTMLInputElement).value.length > 0
        ? ctl.open()
        : ctl.close();
    });
  }
...
export function upgradableMenuItem(
  needUpgrade: boolean,
  action: () => void,
  ...rem: any[]
) {
...
export function upgradeText(needUpgrade: boolean, onClick: () => void) {
  ...
  return menuText(
    dom(
      "span",
      t("* Workspaces are available on team plans. "),
...
export function autocomplete(
  ...
  return weasel.autocomplete(inputElem, choices, {
    ...
    ...options,
    menuCssClass:
      defaults.menuCssClass +
      " " +
      cssSelectMenuElem.className +
      " " +
...
export function selectTitle(
  ...
  return cssOptionRow(
    iconName
      ? dom.domComputed(iconName, (name) => cssOptionRowIcon(name))
      : null,
    dom.text(label)
  );
}
...
export const menuSubHeader = styled(
  "div",
  `
 ...
 * can be removed.
 */
export const menuItemAsync: typeof weasel.menuItem = function (
  action,
  ...args
) {
...
export const menuDivider = styled(
  weasel.cssMenuDivider,
  `
...
const cssSelectMenuElem = styled(
  cssMenuElem,
  `
  max-height: 400px;
  overflow-y: auto;
...
const cssSelectBtnContainer = styled(
  "div",
  `
  position: relative;
  width: 100%;
...
export const cssOptionRow = styled(
  "span",
  `
  display: flex;
  align-items: center;
...
export const cssOptionRowIcon = styled(
  cssOptionIcon,
  `
  margin: 0 8px 0 0;
  flex: none;
...
const cssInlineCollapseIcon = styled(
  icon,
  `
  margin: 0 2px;
  pointer-events: none;
...
const cssInputButtonMenuElem = styled(
  cssMenuElem,
  `
  padding: 4px 0px;
`
...
const cssAnnotateMenuItem = styled(
  "span",
  `
...
const cssMultiSelectSummary = styled(
  "div",
  `
  flex: 1 1 0px;
  overflow: hidden;
...
const cssMultiSelectMenu = styled(
  weasel.cssMenu,
  `
  display: flex;
...
const cssUpgradeTextButton = styled(
  textButton,
  `
  font-size: ${vars.smallFontSize};
`
