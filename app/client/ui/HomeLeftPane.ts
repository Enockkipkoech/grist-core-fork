...
import {
  getWorkspaceInfo,
  workspaceName,
} from "app/client/models/WorkspaceInfo";
...
export function createHomeLeftPane(
  ...
  return cssContent(
    ...
    cssScrollPane(
      ...
      dom.forEach(home.workspaces, (ws) => {
        ...
        const isTrivial = computed((use) =>
          Boolean(
            getWorkspaceInfo(home.app, ws).isDefault &&
        ...
        return cssPageEntry(
          ...
          cssPageEntry.cls(
            "-selected",
            (use) => use(home.currentWSId) === ws.id
          ),
          cssPageLink(
            cssPageIcon("Folder"),
              ...
              cssEditorInput(
                {
                  ...
                  save: async (val) =>
                    val !== ws.name
                      ? home.renameWorkspace(ws.id, val)
                      : undefined,
            ...
            cssEditorInput(
              {
                initialValue: "",
                save: async (val) =>
                  val !== "" ? home.createWorkspace(val) : undefined,
      ...
      cssTools(
        cssPageEntry(
          ...
          cssPageEntry.cls(
            "-selected",
            (use) => use(home.currentPage) === "templates"
          ),
          cssPageLink(
            cssPageIcon("Board"),
            cssLinkText(t("Examples & Templates")),
        ...
        cssPageEntry(
          cssPageEntry.cls(
            "-selected",
            (use) => use(home.currentPage) === "trash"
          ),
          cssPageLink(
            cssPageIcon("RemoveBig"),
            cssLinkText(t("Trash")),
        ...
        cssPageEntry(
          ...
          cssPageLink(
            cssPageIcon("Bookmark"),
            cssLinkText(t("Tutorial")),
...
export async function createDocAndOpen(home: HomeModel) {
  ...
  try {
    const docId = await home.createDoc(
      "Untitled document",
  ...
  return [
    ...
    domComputed(home.importSources, (importSources) => [
      ...importSources.map((source, i) =>
        menuItem(
          () => importFromPluginAndOpen(home, source),
          menuIcon("Import"),
    ...
    upgradableMenuItem(
      ...
      dom.cls(
        "disabled",
        (use) => !roles.canEdit(orgAccess) || !use(home.available)
      ),
...
function workspaceMenu(
  home: HomeModel,
  ws: Workspace,
  renaming: Observable<Workspace | null>
) {
  function deleteWorkspace() {
    confirmModal(
      t("Delete {{workspace}} and all included documents?", {
        workspace: ws.name,
      }),
      t("Delete"),
...
export const cssEditorInput = styled(
  transientInput,
  `
  height: 24px;
  flex: 1 1 0px;
