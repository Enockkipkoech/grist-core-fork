...
export function getRealAccess(
  ...
  const inheritedAccess = roles.getWeakestRole(
    user.parentAccess || null,
    permissionData.maxInheritedRole || null
  );
...
export interface DocSnapshot {
  ...
  docId: string;
  label?: string; // an id for accessing the snapshot as a Grist document
}
...
export class UserAPIImpl extends BaseAPI implements UserAPI {
  ...
  public forRemoved(): UserAPI {
    ...
    return new UserAPIImpl(this._homeUrl, {
      ...this._options,
      extraParameters,
    });
  }
  ...
  public async getSessionActive(): Promise<ActiveSessionInfo> {
    return this.requestJson(`${this._url}/api/session/access/active`, {
      method: "GET",
    });
  }
  ...
  public async getSessionAll(): Promise<{
    users: FullUser[];
    orgs: Organization[];
  }> {
    return this.requestJson(`${this._url}/api/session/access/all`, {
      method: "GET",
    });
  }

  public async getOrgs(merged: boolean = false): Promise<Organization[]> {
    return this.requestJson(`${this._url}/api/orgs?merged=${merged ? 1 : 0}`, {
      method: "GET",
    });
  }

  public async getWorkspace(workspaceId: number): Promise<Workspace> {
    return this.requestJson(`${this._url}/api/workspaces/${workspaceId}`, {
      method: "GET",
    });
  }

  public async getOrg(orgId: number | string): Promise<Organization> {
    return this.requestJson(`${this._url}/api/orgs/${orgId}`, {
      method: "GET",
    });
  }
  ...
  public async getOrgWorkspaces(orgId: number | string): Promise<Workspace[]> {
    return this.requestJson(
      `${this._url}/api/orgs/${orgId}/workspaces?includeSupport=1`,
      { method: "GET" }
    );
  }

  public async getOrgUsageSummary(
    orgId: number | string
  ): Promise<OrgUsageSummary> {
    return this.requestJson(`${this._url}/api/orgs/${orgId}/usage`, {
      method: "GET",
    });
  }
  ...
  public async getTemplates(
    ...
    return this.requestJson(
      `${this._url}/api/templates?onlyFeatured=${onlyFeatured ? 1 : 0}`,
      { method: "GET" }
    );
  }

  public async getWidgets(): Promise<ICustomWidget[]> {
    return await this.requestJson(`${this._url}/api/widgets`, {
      method: "GET",
    });
  }

  public async getDoc(docId: string): Promise<Document> {
    return this.requestJson(`${this._url}/api/docs/${docId}`, {
      method: "GET",
    });
  }
  ...
  public async softDeleteWorkspace(workspaceId: number): Promise<void> {
    await this.request(`${this._url}/api/workspaces/${workspaceId}/remove`, {
      method: "POST",
    });
  }

  public async undeleteWorkspace(workspaceId: number): Promise<void> {
    await this.request(`${this._url}/api/workspaces/${workspaceId}/unremove`, {
      method: "POST",
    });
  }
  ...
  public async softDeleteDoc(docId: string): Promise<void> {
    await this.request(`${this._url}/api/docs/${docId}/remove`, {
      method: "POST",
    });
  }

  public async undeleteDoc(docId: string): Promise<void> {
    await this.request(`${this._url}/api/docs/${docId}/unremove`, {
      method: "POST",
    });
  }
  ...
  public async getOrgAccess(orgId: number | string): Promise<PermissionData> {
    return this.requestJson(`${this._url}/api/orgs/${orgId}/access`, {
      method: "GET",
    });
  }
  ...
  public async getWorkspaceAccess(
    ...
    return this.requestJson(
      `${this._url}/api/workspaces/${workspaceId}/access`,
      { method: "GET" }
    );
  }

  public async getDocAccess(docId: string): Promise<PermissionData> {
    return this.requestJson(`${this._url}/api/docs/${docId}/access`, {
      method: "GET",
    });
  }
  ...
  public async updateAllowGoogleLogin(
    allowGoogleLogin: boolean
  ): Promise<void> {
  ...
  public async updateIsConsultant(
    userId: number,
    isConsultant: boolean
  ): Promise<void> {
...
export class DocWorkerAPIImpl extends BaseAPI implements DocWorkerAPI {
  ...
  public async importDocToWorkspace(
    ...
    return this.requestJson(
      `${this.url}/api/workspaces/${workspaceId}/import`,
      {
        method: "POST",
  ...
  public async downloadDoc(
    ...
    const result = await this.request(
      `${this.url}/api/docs/${docId}/download${extra}`,
      {
        method: "GET",
      }
    );
...
export class DocAPIImpl extends BaseAPI implements DocAPI {
  ...
  public async getRecords(
    ...
    const response: TableRecordValues = await this._getRecords(
      tableId,
      "records",
      options
    );
  ...
  public async addWebhook(
    webhook: WebhookSubscribe & { tableId: string }
  ...
  public getDownloadTableSchemaUrl(params: DownloadDocParams) {
    // We spread `params` to work around TypeScript being overly cautious.
    return (
      this._url + "/download/table-schema?" + encodeQueryParams({ ...params })
    );
  }

  public async sendToDrive(
    code: string,
    title: string
  ): Promise<{ url: string }> {
 ...
 */
export function getDocWorkerUrl(
  homeUrl: string,
  docWorkerInfo: {
