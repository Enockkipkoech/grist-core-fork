...
// Schema validators for api endpoints that creates or updates records.
const { WebhookPatch, WebhookSubscribe } = t.createCheckers(TriggersTI);
...
function validateCore(checker: Checker, req: Request, body: any) {
  try {
    ...
    log.warn(
      `Error during api call to ${req.path}: Invalid payload: ${String(err)}`
    );
...
export class DocWorkerApi {
  ...
  private _dailyUsage = new LRUCache<string, number>({
    max: 5 * MAX_ACTIVE_DOCS_USAGE_CACHE,
  });
  ...
  public addEndpoints() {
    ...
    const canView = expressWrap(
      this._assertAccess.bind(this, "viewers", false)
    );
    // check document exists (not soft deleted) and user can edit it
    const canEdit = expressWrap(
      this._assertAccess.bind(this, "editors", false)
    );
    ...
    const canEditMaybeRemoved = expressWrap(
      this._assertAccess.bind(this, "editors", true)
    );
    ...
    const withDoc = (callback: WithDocHandler) =>
      throttled(this._requireActiveDoc(callback));
    ...
    async function getTableData(
      ...
      const { tableData } = await handleSandboxError(
        tableId,
        [],
        activeDoc.fetchQuery(session, { tableId, filters }, !immediate)
      );
      ...
      const columns = isMetaTable
        ? null
        : await handleSandboxError(
            "",
            [],
            activeDoc.getTableCols(session, tableId, true)
          );
    ...
    async function getTableRecords(
      ...
      const fieldNames = Object.keys(columnData).filter(
        (k) =>
          !(["id", "manualSort"].includes(k) || k.startsWith("gristHelper_"))
      );
    ...
    async function getWebhookSettings(
      ...
      const trigger = webhookId
        ? activeDoc.triggers.getWebhookTriggerRecord(webhookId)
        : undefined;
      let currentTableId = trigger
        ? tablesTable.getValue(trigger.tableRef, "tableId")!
        : undefined;
      ...
      if (isReadyColumn !== undefined) {
        ...
        if (isReadyColumn !== null && isReadyColumn !== "") {
          if (!currentTableId) {
            throw new ApiError(
              `Cannot find column "${isReadyColumn}" because table is not known`,
              404
            );
          }
          fields.isReadyColRef = colIdToReference(
            metaTables,
            currentTableId,
            isReadyColumn
          );
    ...
    this._app.post(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        res.json(
          await activeDoc.addAttachments(
            docSessionFromRequest(req),
    ...
    function cleanAttachmentRecord(
      record: MetaRowRecord<"_grist_Attachments">
    ) {
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const records = rawRecords.map((r) => ({
          id: r.id,
          fields: cleanAttachmentRecord(
            r.fields as MetaRowRecord<"_grist_Attachments">
          ),
        }));
        res.json({ records });
      })
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        if ((tableId || colId || rowId) && !(tableId && colId && rowId)) {
          throw new ApiError(
            "define all of tableId, colId and rowId, or none.",
            400
          );
        }
        const attRecord = activeDoc.getAttachmentMetadata(attId);
        const cell =
          tableId && colId && rowId ? { tableId, colId, rowId } : undefined;
        const fileIdent = attRecord.fileIdent as string;
        const ext = path.extname(fileIdent);
        const origName = attRecord.fileName as string;
        const fileName = ext
          ? path.basename(origName, path.extname(origName)) + ext
          : origName;
        const fileData = await activeDoc.getAttachmentData(
          docSessionFromRequest(req),
          attRecord,
          { cell }
        );
    ...
    // Mostly for testing
    this._app.post(
      "/api/docs/:docId/attachments/updateUsed",
      canEdit,
      withDoc(async (activeDoc, req, res) => {
    ...
    this._app.post(
      "/api/docs/:docId/attachments/verifyFiles",
      isOwner,
      withDoc(async (activeDoc, req, res) => {
        await verifyAttachmentFiles(activeDoc);
        res.json(null);
    ...
    this._app.post(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const { retValues } = await handleSandboxError(
          tableId,
          [],
    ...
    this._app.post(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const actions = body.tables.map(({ columns, id }) => {
          const colInfos = columns.map(({ fields, id: colId }) => ({
            ...fields,
            id: colId,
          }));
          return ["AddTable", id, colInfos];
        });
        const { retValues } = await activeDoc.applyUserActions(
          docSessionFromRequest(req),
          actions
        );
      ...
      throttled(async (req, res) => {
        ...
        if (await this._isOwner(req, { acceptTrunkForSnapshot: true })) {
          ...
          try {
            ...
            return await this._docWorker.downloadDoc(
              req,
              res,
              this._docManager.storageManager
            );
          } catch (e) {
            if (e.message && e.message.match(/does not exist yet/)) {
              ...
              return this._docWorker.downloadDoc(
                req,
                res,
                this._docManager.storageManager
              );
    ...
    this._app.post(
      "/api/docs/:docId/create-fork",
      canEdit,
      throttled(async (req, res) => {
        const docId = stringParam(req.params.docId, "docId");
        const srcDocId = stringParam(req.body.srcDocId, "srcDocId");
        if (srcDocId !== req.specialPermit?.otherDocId) {
          throw new Error("access denied");
        }
        const fname = await this._docManager.storageManager.prepareFork(
          srcDocId,
          docId
        );
    ...
    this._app.patch(
      "/api/docs/:docId/tables/:tableId/columns",
      canEdit,
      validate(ColumnsPatch),
      withDoc(async (activeDoc, req, res) => {
        const tablesTable = activeDoc.docData!.getMetaTable("_grist_Tables");
        const columnsTable = activeDoc.docData!.getMetaTable(
        ...
        const columns: Types.Record[] = body.columns.map((col) => {
          const id = columnsTable.findMatchingRowId({
            parentId: tableRef,
            colId: col.id,
          });
          if (!id) {
    ...
    this._app.post(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const webhookId = (
          await this._dbManager.addSecret(secretValue, activeDoc.docName)
        ).id;
        ...
        try {
          ...
          const sandboxRes = await handleSandboxError(
            "_grist_Triggers",
            [],
            activeDoc.applyUserActions(docSessionFromRequest(req), [
              [
                "AddRecord",
                "_grist_Triggers",
                null,
          ...
          // remove webhook
          await this._dbManager.removeWebhook(
            webhookId,
            activeDoc.docName,
            "",
            false
    ...
    this._app.post(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const triggerRowId = activeDoc.triggers.getWebhookTriggerRecord(
          webhookId,
          tableRef
        ).id;
    ...
    this._app.patch(
      ...
      isOwner,
      validate(WebhookPatch),
      withDoc(async (activeDoc, req, res) => {
        const docId = activeDoc.docName;
        const webhookId = req.params.webhookId;
        const { fields, trigger, url } = await getWebhookSettings(
          activeDoc,
          req,
          webhookId
        );
        ...
        const triggerRowId =
          activeDoc.triggers.getWebhookTriggerRecord(webhookId).id;

        await this._dbManager.connection.transaction(async (manager) => {
          // update url
          if (url) {
            await this._dbManager.updateWebhookUrl(
              webhookId,
              docId,
              url,
              manager
    ...
    this._app.post(
      ...
      throttled(async (req, res) => {
        ...
        const recoveryMode =
          typeof recoveryModeRaw === "boolean" ? recoveryModeRaw : undefined;
        if (!(await this._isOwner(req))) {
          throw new Error("Only owners can control recovery mode");
        }
        this._docManager.setRecovery(getDocId(req), recoveryMode ?? true);
        const activeDoc = await this._docManager.fetchDoc(
          docSessionFromRequest(req),
          getDocId(req),
          recoveryMode
        );
    ...
    this._app.post(
      ...
      throttled(async (req, res) => {
        ...
        if (req.body.sourceDocId) {
          options.sourceDocId = await this._confirmDocIdForRead(
            req,
            String(req.body.sourceDocId)
          );
          // Make sure that if we wanted to download the full source, we would be allowed.
          const result = await fetch(
            this._grist.getHomeUrl(
              req,
              `/api/docs/${options.sourceDocId}/download?dryrun=1`
            ),
            {
              method: "GET",
              headers: {
          ...
          await fetch(
            this._grist.getHomeUrl(
              req,
              `/api/docs/${options.sourceDocId}/flush`
            ),
            {
              method: "POST",
              headers: {
          ...
          if (req.body.resetTutorialMetadata) {
            ...
            await this._dbManager.connection.transaction(async (manager) => {
              ...
              const tutorialTrunk = await this._dbManager.getRawDocById(
                tutorialTrunkId,
                manager
              );
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const ref = await fetch(
          this._grist.getHomeUrl(req, `/api/docs/${req.params.docId2}/states`),
          {
            headers: {
              ...getTransitiveHeaders(req),
              "Content-Type": "application/json",
        ...
        if (showDetails && parent) {
          ...
          const rightChangesReq = await fetch(
            this._grist.getHomeUrl(req, url),
            {
              headers: {
                ...getTransitiveHeaders(req),
                "Content-Type": "application/json",
          ...
          const rightChanges = (await rightChangesReq.json()).details!
            .rightChanges;
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const { states } = await this._getStates(docSession, activeDoc);
        res.json(
          await this._getChanges(docSession, activeDoc, states, left, right)
        );
      })
    );
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const tableSchema = await collectTableSchemaInFrictionlessFormat(
          activeDoc,
          req,
          options
        );
    ...
    this._app.get(
      ...
      withDoc(async (activeDoc, req, res) => {
        ...
        const { name: docTitle } = await this._dbManager.getDoc(req);
        const options = !_.isEmpty(req.query)
          ? this._getDownloadOptions(req, docTitle)
          : {
              filename: docTitle,
              tableId: "",
    ...
    this._app.post(
      ...
      expressWrap(async (req, res) => {
        ...
        if (parameters.workspaceId) {
          throw new Error("workspaceId not supported");
        }
        const browserSettings: BrowserSettings = {};
        if (parameters.timezone) {
          browserSettings.timezone = parameters.timezone;
        }
   ...
   * request.
   */
  private async _confirmDocIdForRead(
    req: Request,
    urlId: string
  ): Promise<string> {
    const docAuth = await makeDocAuthResult(
      this._dbManager.getDoc({ ...getScope(req), urlId })
    );
    if (docAuth.error) {
      throw docAuth.error;
    }
  ...
  private _getActiveDocIfAvailable(
    req: RequestWithLogin
  ): Promise<ActiveDoc> | undefined {
    ...
    return async (req, res, next) => {
      ...
      try {
        ...
        if (count + 1 > MAX_PARALLEL_REQUESTS_PER_DOC) {
          throw new ApiError(
            `Too many backlogged requests for document ${docId} - ` +
              `try again later?`,
            429
          );
        }
   ...
   */
  private async _checkDailyDocApiUsage(
    req: Request,
    docId: string
  ): Promise<boolean> {
    ...
    const max =
      doc.workspace.org.billingAccount?.product.features
    ...
    if (!cli) {
      throw new Error("redis unexpectedly not available");
    }
    ...
    multi
      ...
      .catch((e) =>
        console.error(`Error tracking API usage for doc ${docId}`, e)
      );
  ...
  private async _assertAccess(
    ...
    const docAuth = await getOrSetDocAuth(
   ...
   */
  private async _isOwner(
    req: Request,
    options?: { acceptTrunkForSnapshot?: boolean }
  ) {
    const scope = getDocScope(req);
    const docAuth = await getOrSetDocAuth(
      req as RequestWithLogin,
    ...
    if (
      options?.acceptTrunkForSnapshot &&
      docAuth.cachedDoc?.trunkAccess === "owners"
    ) {
  ...
  private async _getChanges(
    ...
    if (rightOffset > leftOffset) {
      throw new Error(
        "Comparisons currently require left to be an ancestor of right"
      );
    }
    const actionNums: number[] = states
      .slice(rightOffset, leftOffset)
      .map((state) => state.n);
    ...
    if (permanent) {
      ...
      const docsToDelete = [
        ...
        ...forks.map((fork) =>
          buildUrlId({
            forkId: fork.id,
            forkUserId: fork.createdBy!,
            trunkId: docId,
          })
        ),
      ];
      await Promise.all(
        docsToDelete.map((docName) =>
          this._docManager.deleteDoc(null, docName, true)
        )
      );
...
function getSortParameter(req: Request): string[] | undefined {
  const sortString: string | undefined =
    optStringParam(req.query.sort) || req.get("X-Sort");
  if (!sortString) {
    return undefined;
  }
...
function getLimitParameter(req: Request): number | undefined {
  const limitString: string | undefined =
    optStringParam(req.query.limit) || req.get("X-Limit");
  if (!limitString) {
    return undefined;
  }
...
function applySort(
  ...
  if (!_columns) {
    _columns = Object.keys(values).map((col, index) => ({
      id: col,
      fields: { colRef: index },
    }));
  }
  ...
  const colIdToRef = new Map(
    properColumns.map(({ id, colId }) => [colId as string, id as number])
  );
...
export function applyQueryParameters(
  ...
  if (params.sort) {
    applySort(values, params.sort, columns);
  }
  if (params.limit) {
    applyLimit(values, params.limit);
  }
  return values;
}
 ...
 */
export function docPeriodicApiUsageKey(
  docId: string,
  current: boolean,
  period: DocApiUsagePeriod,
  m: moment.Moment
) {
  if (!current) {
...
export function getDocApiUsageKeysToIncr(
  docId: string,
  usage: LRUCache<string, number>,
  dailyMax: number,
  m: moment.Moment
): string[] | undefined {
  // Start with keys for the current day, minute, and hour
  const keys = docApiUsagePeriods.map((p) =>
    docPeriodicApiUsageKey(docId, true, p, m)
  );
