...
} from "app/common/AttachmentColumns";
import {
  BulkAddRecord,
  BulkRemoveRecord,
  BulkUpdateRecord,
...
import {
  ...
  sendForCompletion,
} from "app/server/lib/Assistance";
...
import {
  GRIST_DOC_SQL,
  GRIST_DOC_WITH_TABLE1_SQL,
} from "app/server/lib/initialDocSql";
import { ISandbox } from "app/server/lib/ISandbox";
import log from "app/server/lib/log";
import { LogMethods } from "app/server/lib/LogMethods";
import {
  NullSandbox,
  UnavailableSandboxMethodError,
} from "app/server/lib/NullSandbox";
...
import {
  getLogMetaFromDocSession,
  getPubSubPrefix,
  getTelemetryMetaFromDocSession,
  timeoutReached,
} from "./serverUtils";
...
const DEFAULT_TIMEZONE = (process.versions as any).electron
  ? moment.tz.guess()
  : "UTC";
...
const UPDATE_CURRENT_TIME_DELAY = {
  delayMs: 60 * 60 * 1000,
  varianceMs: 30 * 1000,
};

// Measure and broadcast data size every 5 minutes
const UPDATE_DATA_SIZE_DELAY = {
  delayMs: 5 * 60 * 1000,
  varianceMs: 30 * 1000,
};

// Log document metrics every hour
const LOG_DOCUMENT_METRICS_DELAY = {
  delayMs: 60 * 60 * 1000,
  varianceMs: 30 * 1000,
};
...
export class ActiveDoc extends EventEmitter implements AssistanceDoc {
  ...
  public static keepDocOpen(
    ...
    descriptor.value = function (this: ActiveDoc) {
      return this._inactivityTimer.disableUntilFinish(
  ...
  private _log = new LogMethods("ActiveDoc ", (s: OptDocSession | null) =>
    this.getLogMeta(s)
  );
  ...
  private _fetchCache = new MapWithTTL<string, Promise<TableDataAction>>(
    DEFAULT_CACHE_TTL
  );
  ...
  constructor(
    ...
    if (!this._isSnapshot) {
      ...
      this._intervals.push(
        ...
        new Interval(
          ...
          {
            onError: (e) =>
              this._log.error(null, "failed to remove expired attachments", e),
          }
        ),
        // Update the time in formulas every hour.
        new Interval(
          () =>
            this._applyUserActions(makeExceptionalDocSession("system"), [
              ["UpdateCurrentTime"],
            ]),
          UPDATE_CURRENT_TIME_DELAY,
          {
            onError: (e) =>
              this._log.error(null, "failed to update current time", e),
          }
        ),
        // Measure and broadcast data size every 5 minutes.
        new Interval(
          ...
          {
            onError: (e) =>
              this._log.error(null, "failed to update data size", e),
          }
        ),
        // Log document metrics every hour.
        new Interval(
          ...
          {
            onError: (e) =>
              this._log.error(null, "failed to log document metrics", e),
          }
        )
      );
    }
    ...
    this.docPluginManager = docManager.pluginManager
      ? new DocPluginManager(
          docManager.pluginManager.getPlugins(),
          docManager.pluginManager.appRoot!,
          this,
          this._docManager.gristServer
        )
      : null;
  ...
  public get recoveryMode(): boolean {
  ...
  public get triggers(): DocTriggers {
  ...
  public async getFilteredDocUsageSummary(
    ...
    return this._granularAccess.filterDocUsageSummary(
      docSession,
      this.getDocUsageSummary()
    );
  }
  ...
  public getRecentActionsDirect(
    maxActions?: number
  ): Promise<LocalActionBundle[]> {
    return this._actionHistory.getRecentActions(maxActions);
  }

  public async getRecentStates(
    docSession: OptDocSession,
    maxStates?: number
  ): Promise<DocState[]> {
  ...
  public getActions(
    actionNums: number[]
  ): Promise<Array<LocalActionBundle | undefined>> {
  ...
  public async getRecentActions(
    ...
    const groups = await this._actionHistory.getRecentActionGroups(
      MAX_RECENT_ACTIONS,
  ...
  public async getRecentMinimalActions(
    docSession: OptDocSession
  ): Promise<MinimalActionGroup[]> {
  ...
  public addClient(client: Client, authorizer: Authorizer): DocSession {
    const docSession: DocSession = this.docClients.addClient(
      client,
      authorizer
    );
  ...
  public async createEmptyDocWithDataEngine(
    docSession: OptDocSession
  ): Promise<ActiveDoc> {
    ...
    const initBundle = await this._rawPyCall("apply_user_actions", [
      ["InitNewDoc"],
    ]);
  ...
  public async loadDoc(
    ...
    try {
      ...
      const desiredTableNames = tableNames.filter((name) =>
        name.startsWith("_grist_")
      );
      this._startLoadingTables(docSession, desiredTableNames);
      const pendingTableNames = tableNames.filter(
        (name) => !name.startsWith("_grist_")
      );
      await this._initDoc(docSession);
      this._initializationPromise = this._finishInitialization(
        docSession,
        pendingTableNames,
        onDemandNames,
        startTime
  ...
  public async replace(
    ...
    return this.shutdown({
      afterShutdown: () =>
        this._docManager.storageManager.replace(this.docName, source),
    });
  }
  ...
  public async _initDoc(docSession: OptDocSession): Promise<void> {
    const metaTableData =
      await this._tableMetadataLoader.fetchTablesAsActions();
    this.docData = new DocData(
      (tableId) =>
        this.fetchTable(makeExceptionalDocSession("system"), tableId),
      metaTableData
    );
    this._onDemandActions = new OnDemandActions(
      this.docStorage,
      this.docData,
      this._recoveryMode
    );
    ...
    this._granularAccess = new GranularAccess(
      this.docData,
      this.docStorage,
      this.docClients,
      (query) => {
  ...
  public importFiles(
    ...
    return this._activeDocImport.importFiles(
      docSession,
      dataSource,
      parseOptions,
      prevTableIds
   ...
   * call, or empty if there was no previous call.
   */
  public finishImportFiles(
    docSession: DocSession,
    dataSource: DataSourceTransformed,
    prevTableIds: string[],
    importOptions: ImportOptions
  ): Promise<ImportResult> {
    return this._activeDocImport.finishImportFiles(
      docSession,
      dataSource,
      prevTableIds,
      importOptions
   ...
   * call, or empty if there was no previous call.
   */
  public cancelImportFiles(
    docSession: DocSession,
    uploadId: number,
    prevTableIds: string[]
  ): Promise<void> {
    return this._activeDocImport.cancelImportFiles(
      docSession,
      uploadId,
      prevTableIds
    );
  ...
  public async closeDoc(docSession: DocSession): Promise<void> {
    ...
    if (this.docClients.clientCount() === 0) {
      this._log.info(
        docSession,
        "will self-close in %d ms",
        this._inactivityTimer.getDelay()
      );
      this._inactivityTimer.enable();
    }
  }
  ...
  public async addAttachments(
    ...
    const upload: UploadInfo = globalUploadSet.getUploadInfo(
      uploadId,
      this.makeAccessId(userId)
    );
    try {
        ...
        async (e) => {
          ...
          if (hadChanges) {
            ...
            throw new LimitExceededError(
              "Exceeded attachments limit for document"
            );
          }
        }
      );
      ...
      const result = await this._applyUserActionsWithExtendedOptions(
        docSession,
        userActions,
        {
          attachment: true,
  ...
  public getAttachmentMetadata(
    attId: number
  ): MetaRowRecord<"_grist_Attachments"> {
    // docData should always be available after loadDoc() or createDoc().
    if (!this.docData) {
      throw new Error("No doc data");
    }
    const attRecord = this.docData
      .getMetaTable("_grist_Attachments")
      .getRecord(attId);
    if (!attRecord) {
  ...
  public async getAttachmentData(
    docSession: OptDocSession,
    attRecord: MetaRowRecord<"_grist_Attachments">,
    options?: {
      cell?: SingleCell;
      maybeNew?: boolean;
    }
    ...
    if (
      ...
      if (
        maybeNew &&
        (await this._granularAccess.isAttachmentUploadedByUser(
          docSession,
          attId
        ))
      ) {
        ...
        await this._granularAccess.assertAttachmentAccess(
          docSession,
          cell,
          attId
        );
        ...
        if (
          !(await this._granularAccess.findAttachmentCellForUser(
            docSession,
            attId
          ))
        ) {
    ...
    if (!data) {
      throw new ApiError("Invalid attachment identifier", 404);
    }
    this._log.info(
      docSession,
      "getAttachment: %s -> %s bytes",
      fileIdent,
      data.length
    );
    return data;
   ...
   */
  public async fetchTable(
    docSession: OptDocSession,
    tableId: string,
    waitForFormulas: boolean = false
  ...
  public async fetchQuery(
    ...
    const tableAccess = await this._granularAccess.getTableAccess(
      docSession,
      query.tableId
    );
    ...
    if (
      query.tableId.startsWith("_grist_") &&
      !(await this._granularAccess.canReadEverything(docSession))
    ) {
    ...
    if (onDemand || this._isSnapshot) {
      ...
      if (this._fullyLoaded) {
        // Already loaded or finished loading while fetching from DB
        const key = JSON.stringify(query);
        // TODO: cache longer if the underlying fetch takes longer to do.
        data = await mapGetOrSet(this._fetchCache, key, () =>
          this._fetchQueryFromDataEngine(query)
        );
      }
    }
    ...
    if (
      ...
      (await this._granularAccess.needAttachmentControl(docSession))
    ) {
      const attIds = gatherAttachmentIds(attachmentColumns, data!);
      if (attIds.size > 0) {
        attachments = this.docData!.getMetaTable(
          "_grist_Attachments"
    ...
    this._log.info(
      docSession,
      "fetchQuery -> %d rows, cols: %s",
      data![2].length,
      Object.keys(data![3]).join(", ")
    );
  ...
  public async fetchTableSchema(docSession: DocSession): Promise<string> {
    ...
    if (
      ...
      throw new ApiError(
        "Cannot view code, it may contain private material",
        403
      );
    }
    await this.waitForInitialization();
   ...
   */
  public async findColFromValues(
    docSession: DocSession,
    values: any[],
    n: number,
    optTableId?: string
  ...
  public async getTableCols(
    ...
    (columnData.colId as string[]).forEach((id, index) => {
      const hasNoId = !id;
      const isHidden =
        hasNoId || id === "manualSort" || id.startsWith("gristHelper_");
      const fromDifferentTable = columnData.parentId[index] !== tableRef;
      const skip =
        (isHidden && !includeHidden) || hasNoId || fromDifferentTable;
      if (skip) {
        return;
      }
      const column: TableRecordValue = {
        id,
        fields: { colRef: colRefs[index] },
      };
  ...
  public async getFormulaError(
    ...
    await this._granularAccess.getCellValue(docSession, {
      tableId,
      colId,
      rowId,
    });
  ...
  public async applyUserActionsById(
    ...
    for (const [index, bundle] of actionBundles.entries()) {
      ...
      if (!bundle) {
        throw new Error(`Could not find actionNum ${actionNum}`);
      }
      ...
      if (actionHash !== bundle.actionHash) {
        throw new Error(
          `Hash mismatch for actionNum ${actionNum}: ` +
    ...
    if (undo) {
      actions = [
        ["ApplyUndoActions", flatten(actionBundles.map((a) => a!.undo))],
      ];
    } else {
    ...
    return this._applyUserActionsWithExtendedOptions(docSession, actions, {
      bestEffort: undo,
      oldestSource,
      fromOwnHistory,
      ...(options || {}),
   ...
   */
  public async processActionBundle(
    localActionBundle: LocalActionBundle
  ): Promise<void> {
    ...
    if (
      docActions.some((docAction) =>
        this._onDemandActions.isSchemaAction(docAction)
      )
    ) {
  ...
  public async getAssistanceWithOptions(
    docSession: DocSession,
    request: AssistanceRequest
  ): Promise<AssistanceResponse> {
  ...
  public assistanceSchemaPromptV1(
  ...
  public fetchURL(
    ...
    return fetchURL(
      url,
      this.makeAccessId(docSession.authorizer.getUserId()),
      options
    );
  }

  public async forwardPluginRpc(
    docSession: DocSession,
    pluginId: string,
    msg: IMessage
  ): Promise<any> {
    ...
    switch (msg.mtype) {
      case MsgType.RpcCall:
        return pluginRpc.forwardCall(msg);
      case MsgType.Custom:
        return pluginRpc.forwardMessage(msg);
    }
  ...
  public async reloadPlugins(docSession: DocSession) {
    ...
    if (!this._docManager.pluginManager || !this.docPluginManager) {
      return;
    }
  ...
  public async reloadDoc(docSession?: DocSession) {
    this._log.debug(
      docSession || null,
      "ActiveDoc.reloadDoc starting shutdown"
    );
    return this.shutdown();
  }
  ...
  public async fork(docSession: OptDocSession): Promise<ForkResult> {
    ...
    if (!user || !(await this.canDownload(docSession))) {
      throw new ApiError(
        "Insufficient access to document to copy it entirely",
        403
      );
    }
    ...
    try {
      ...
      const isTemplate =
        TEMPLATES_ORG_DOMAIN === doc.workspace.org.domain &&
        doc.type !== "tutorial";
  ...
  public async getAccessToken(
    ...
    if (!userId) {
      throw new Error("creating access token requires a user");
    }
    ...
    if (!baseUrl) {
      throw new Error("cannot create token without URLs");
    }
    return {
      token,
      baseUrl,
   ...
   */
  public async checkAclFormula(
    docSession: DocSession,
    text: string
  ): Promise<FormulaProperties> {
  ...
  public async getAclResources(docSession: DocSession): Promise<AclResources> {
    if (
      !this.docData ||
      !(await this._granularAccess.hasAccessRulesPermission(docSession))
    ) {
    ...
    for (const table of tables.getRecords()) {
      const sourceTable = table.summarySourceTable
   ...
   */
  public async getUsersForViewAs(
    docSession: OptDocSession
  ): Promise<PermissionDataWithExtraUsers> {
    ...
    if (db) {
      const access = db.unwrapQueryResult(
        await db.getDocAccess(
          { userId, urlId: this.docName },
          {
            flatten: true,
            excludeUsersWithoutAccess: true,
          }
        )
      );
    ...
    // shared with.
    const usersFromUserAttributes =
      await this._granularAccess.collectViewAsUsersFromUserAttributeTables();
    for (const user of usersFromUserAttributes) {
      ...
      if (!isShared.has(email)) {
        result.attributeTableUsers.push({
          ...
          name: user.name || "",
          id: 0,
          access: user.access === undefined ? "editors" : user.access,
        });
      }
    }
  ...
  public async getActionSummaries(
    docSession: DocSession
  ): Promise<ActionGroup[]> {
  ...
  public async applyActionsToDataEngine(
    ...
    const [normalActions, onDemandActions] =
      this._onDemandActions.splitByStorage(userActions);
    ...
    if (normalActions.length > 0) {
      ...
      const user = docSession
        ? await this._granularAccess.getCachedUser(docSession)
        : undefined;
      sandboxActionBundle = await this._rawPyCall(
        "apply_user_actions",
        normalActions,
        user?.toJSON()
      );
      ...
      if (requests) {
        this._requests
          .handleRequestsBatchFromUserActions(requests)
    ...
    if (onDemandActions.length > 0) {
      ...
      await this.docStorage.execTransaction(async () => {
        for (const action of onDemandActions) {
          const { stored, undo, retValues } =
            await this._onDemandActions.processUserAction(action);
          // Note: onDemand stored/undo actions are arbitrarily processed/added after normal actions
          // and do not support access control.
          sandboxActionBundle.stored.push(
            ...stored.map((a) => [allIndex, a] as [number, DocAction])
          );
          sandboxActionBundle.direct.push(
            ...stored.map((a) => [allIndex, true] as [number, boolean])
          );
          sandboxActionBundle.undo.push(
            ...undo.map((a) => [allIndex, a] as [number, DocAction])
          );
  ...
  public async removeUnusedAttachments(
    ...
    const rowIds = await this.docStorage.getSoftDeletedAttachmentIds(
      expiredOnly
    );
    if (rowIds.length) {
      ...
      await this.applyUserActions(makeExceptionalDocSession("system"), [
        action,
      ]);
    }
    try {
  ...
  public async getSnapshots(
    ...
    return this._docManager.storageManager.getSnapshots(
      this.docName,
      skipMetadataCache
    );
  }
  ...
  public async removeSnapshots(
    ...
    return this._docManager.storageManager.removeSnapshots(
      this.docName,
      snapshotIds
    );
  }
   ...
   * database also.
   */
  public async applyStoredActionsToDocStorage(
   ...
   */
  public getGranularAccessForBundle(
    docSession: OptDocSession,
    docActions: DocAction[],
    undo: DocAction[],
    userActions: UserAction[],
    isDirect: boolean[],
    options: ApplyUAOptions | null
  ): GranularAccessForBundle {
    this._granularAccess.getGranularAccessForBundle(
      docSession,
      docActions,
      undo,
      userActions,
  ...
  public async updateRowCount(
    ...
    // the same time.
    if (
      (this.dataSizeLimitRatio > APPROACHING_LIMIT_RATIO &&
        this.rowLimitRatio <= APPROACHING_LIMIT_RATIO) ||
  ...
  public logTelemetryEvent(
    ...
    this._docManager.gristServer
      .getTelemetry()
      .logEvent(event, merge(this._getTelemetryMeta(docSession), metadata))
      .catch((e) =>
        this._log.error(docSession, `failed to log telemetry event ${event}`, e)
      );
  }
  ...
  protected async _loadOpenDoc(docSession: OptDocSession): Promise<string[][]> {
    ...
    const docInfo =
      await this._tableMetadataLoader.fetchBulkColValuesWithoutIds(
        "_grist_DocInfo"
      );
    const versionCol = docInfo.schemaVersion;
    const docSchemaVersion = (
      versionCol && versionCol.length === 1 ? versionCol[0] : 0
    ) as number;
    if (docSchemaVersion < schemaVersion) {
      this._log.info(
        docSession,
        "Doc needs migration from v%s to v%s",
        docSchemaVersion,
        schemaVersion
      );
      ...
      this._log.warn(
        docSession,
        "Doc is newer (v%s) than this version of Grist (v%s); " +
    ...
    const tableNames = (tablesParsed.tableId as string[])
      ...
      .filter(
        (tableId) =>
          tableId !== "_grist_Tables" && tableId !== "_grist_Tables_column"
      )
      .sort();

    // Figure out which tables are on-demand.
    const onDemandMap = zipObject(
      tablesParsed.tableId as string[],
      tablesParsed.onDemand
    );
    const onDemandNames = remove(
      tableNames,
      (t) => onDemandMap[t] || (this._recoveryMode && !t.startsWith("_grist_"))
    );

    this._log.debug(
      docSession,
      "Loading %s normal tables, skipping %s on-demand tables",
      tableNames.length,
      onDemandNames.length
    );
   ...
   */
  @ActiveDoc.keepDocOpen
  protected async _applyUserActions(
    docSession: OptDocSession,
    actions: UserAction[],
    ...
    if (options?.bestEffort) {
      actions = await this._granularAccess.prefilterUserActions(
        docSession,
        actions,
        options
      );
  ...
  private async _doShutdownImpl(): Promise<void> {
    ...
    try {
      ...
      if (this.docClients.clientCount() > 0) {
        this._log.warn(
          docSession,
          `Doc being closed with ${this.docClients.clientCount()} clients left`
        );
      ...
      this._redisSubscriber
        ?.quitAsync()
        .catch((e) =>
          this._log.warn(docSession, "Failed to quit redis subscriber", e)
        );
      ...
      if (this.docStorage.isInitialized()) {
        ...
        try {
          ...
          this._log.error(
            docSession,
            "Failed to remove expired attachments",
            e
          );
        }
      ...
      try {
        ...
        log.error(
          "Problem shutting down document: %s %s",
          this.docName,
          err.message
        );
      }
  ...
  private async _applyUserActionsWithExtendedOptions(
    docSession: OptDocSession,
    actions: UserAction[],
    options?: ApplyUAExtendedOptions
  ): Promise<ApplyUAResult> {
  ...
  private async _createDocFile(
    ...
    const sql = options?.skipInitialTable
      ? GRIST_DOC_SQL
      : GRIST_DOC_WITH_TABLE1_SQL;
    ...
    if (pythonVersion) {
      if (pythonVersion !== "2" && pythonVersion !== "3") {
        throw new Error(
          `PYTHON_VERSION_ON_CREATION must be 2 or 3, not: ${pythonVersion}`
        );
      }
      documentSettings.engine = pythonVersion === "2" ? "python2" : "python3";
    }
    await this.docStorage.run(
      "UPDATE _grist_DocInfo SET timezone = ?, documentSettings = ?",
      timezone,
   ...
   */
  private async _updateDocUsage(
    usage: Partial<DocumentUsage>,
    options: UpdateUsageOptions = {}
  ) {
    const { syncUsageToDatabase = true, broadcastUsageToClients = true } =
      options;
    ...
    if (syncUsageToDatabase) {
      ...
      const didStatusDecrease =
        getSeverity(this.dataLimitStatus) < getSeverity(oldStatus);
  ...
  private _syncDocUsageToDatabase(minimizeDelay = false) {
    this._docManager.storageManager.scheduleUsageUpdate(
      this._docName,
      this._docUsage,
      minimizeDelay
    );
  ...
  private async _broadcastDocUsageToClients() {
    if (this.muted || this.docClients.clientCount() === 0) {
      return;
    }
    ...
    await this.docClients.broadcastDocMessage(
      ...
      async (session, data) => {
        return {
          ...data,
          docUsage: await this.getFilteredDocUsageSummary(session),
        };
      }
    );
  }
  ...
  private async _updateGracePeriodStart(gracePeriodStart: Date | null) {
    this._gracePeriodStart = gracePeriodStart;
    if (!this._isForkOrSnapshot) {
      await this.getHomeDbManager()?.setDocGracePeriodStart(
        this.docName,
        gracePeriodStart
      );
    }
  ...
  private async _prepAttachment(
    ...
    if (fileData.size > limit) {
      throw new ApiError(
        `Attachments must not exceed ${byteString(limit)}`,
        413
      );
    }
    ...
    try {
      dimensions = await bluebird.fromCallback((cb: any) =>
        imageSize(fileData.absPath, cb)
      );
    } catch (err) {
    ...
    const ret: boolean = await this.docStorage.findOrAttachFile(
      fileData.absPath,
      fileIdent
    );
    this._log.info(
      docSession,
      "addAttachment: file %s (image %sx%s) %s",
      fileIdent,
      dimensions.width,
      dimensions.height,
      ret ? "attached" : "already exists"
    );
    return [
      "AddRecord",
      "_grist_Attachments",
  ...
  private async _migrate(docSession: OptDocSession): Promise<void> {
    ...
    this._log.info(
      docSession,
      "_migrate: applying %d migration actions (processed %s schema, %s user tables)",
      docActions.length,
      numSchema,
      numUser
    );

    docActions.forEach((action, i) =>
      this._log.info(
        docSession,
        "_migrate: docAction %s: %s",
        i,
        shortDesc(action)
      )
    );
    await this.docStorage.execTransaction(() =>
      this.docStorage.applyStoredActions(docActions)
    );
  }
    ...
    await bluebird.map(
      ...
      async (tableName: string) =>
        this._pyCall(
          "load_table",
          tableName,
          await this._fetchTableIfPresent(tableName)
        ),
  ...
  private async _finishInitialization(
    docSession: OptDocSession,
    pendingTableNames: string[],
    onDemandNames: string[],
    startTime: number
  ): Promise<void> {
    try {
      ...
      if (this._isSnapshot) {
        ...
        await this._applyUserActions(makeExceptionalDocSession("system"), [
          ["Calculate"],
        ]);
        await this._reportDataEngineMemory();
      }
      ...
      this._log.debug(
        docSession,
        `loaded in ${loadMs} ms, InactivityTimer set to ${closeTimeout} ms`
      );
      ...
      if (!this._shuttingDown) {
        this._log.warn(
          docSession,
          "_finishInitialization stopped with %s",
          err
        );
  ...
  private _logDocMetrics(
    docSession: OptDocSession,
    triggeredBy: "docOpen" | "interval" | "docClose"
  ) {
  ...
  private _getAccessRuleMetrics() {
    ...
    const numUserAttributes =
      accessRules?.getRecords().filter((r) => r.userAttributes).length ?? 0;
  ...
  private _getChartMetrics() {
    ...
    const chartRecords =
      viewSectionRecords?.filter((r) => r.parentKey === "chart") ?? [];
    const chartTypes = chartRecords.map((r) => r.chartType || "bar");
    const numCharts = chartRecords.length;
    const numLinkedCharts = chartRecords.filter(
      (r) => r.linkSrcSectionRef
    ).length;
  ...
  private _getWidgetMetrics() {
    const viewSections = this.docData?.getMetaTable("_grist_Views_section");
    const viewSectionRecords = viewSections?.getRecords() ?? [];
    const numLinkedWidgets = viewSectionRecords.filter(
      (r) => r.linkSrcSectionRef
    ).length;
  ...
  private _getColumnMetrics() {
    const columns = this.docData?.getMetaTable("_grist_Tables_column");
    const columnRecords =
      columns?.getRecords().filter((r) => !isHiddenCol(r.colId)) ?? [];
    const numColumns = columnRecords.length;
    const numColumnsWithConditionalFormatting = columnRecords.filter(
      (r) => r.rules
    ).length;
    const numFormulaColumns = columnRecords.filter(
      (r) => r.isFormula && r.formula
    ).length;
    const numTriggerFormulaColumns = columnRecords.filter(
      (r) => !r.isFormula && r.formula
    ).length;
    ...
    const numSummaryFormulaColumns = columnRecords.filter(
      (r) => r.isFormula && summaryTableIds.has(r.parentId)
    ).length;

    const viewSectionFields = this.docData?.getMetaTable(
      "_grist_Views_section_field"
    );
    const viewSectionFieldRecords = viewSectionFields?.getRecords() ?? [];
    const numFieldsWithConditionalFormatting = viewSectionFieldRecords.filter(
      (r) => r.rules
    ).length;
  ...
  private _getTableMetrics() {
    ...
    const viewSections = this.docData?.getMetaTable("_grist_Views_section");
    const viewSectionRecords = viewSections?.getRecords() ?? [];
    const numTablesWithConditionalFormatting = viewSectionRecords.filter(
      (r) => r.rules
    ).length;
  ...
  private _getCustomWidgetMetrics() {
    ...
    for (const r of viewSectionRecords) {
      ...
      const isGristLabsWidget = url.startsWith(
        commonUrls.gristLabsCustomWidgets
      );
  ...
  private async _fetchQueryFromDB(
    query: ServerQuery,
    onDemand: boolean
    ...
    const table = this.docStorage.decodeMarshalledData(
      marshalled,
      query.tableId
    );
  ...
  private async _fetchQueryFromDataEngine(
    query: ServerQuery
  ): Promise<TableDataAction> {
  ...
  private _getTelemetryMeta(
    docSession: OptDocSession | null
  ): TelemetryMetadataByLevel {
    const altSessionId = docSession
      ? getDocSessionAltSessionId(docSession)
      : undefined;
    return merge(
   ...
   */
  private async _beforeMigration(
    docSession: OptDocSession,
    versionType: "storage" | "schema",
    ...
    this._log.info(
      docSession,
      "_beforeMigration: backup made with label %s at %s",
      label,
      location
    );
  ...
  private async _rawPyCall(
    ...
    try {
      ...
      if (e instanceof UnavailableSandboxMethodError && this._isSnapshot) {
        throw new UnavailableSandboxMethodError(
  ...
  private _pyCall(funcName: string, ...varArgs: unknown[]): Promise<any> {
    return this._modificationLock.runExclusive(() =>
      this._rawPyCall(funcName, ...varArgs)
    );
  }
  ...
  private async _getEngine(): Promise<ISandbox> {
    ...
    this._dataEngine = this._isSnapshot
      ? this._makeNullEngine()
      : this._makeEngine();
    return this._dataEngine;
  }
  ...
  private async _makeEngine(): Promise<ISandbox> {
    ...
    let preferredPythonVersion: "2" | "3" =
      process.env.PYTHON_VERSION === "3" ? "3" : "2";
    ...
    const docInfo = await this.docStorage
      .get("SELECT documentSettings FROM _grist_DocInfo")
      .catch((e) => undefined);
    const docSettingsString = docInfo?.documentSettings;
    if (docSettingsString) {
      const docSettings: DocumentSettings | undefined = safeJsonParse(
        docSettingsString,
        undefined
      );
    ...
    return this._docManager.gristServer.create.NSandbox({
      ...
      sandboxOptions: {
        exports: {
          request: (key: string, args: SandboxRequest) =>
            this._requests.handleSingleRequestWithCache(key, args),
          guessColInfo,
          convertFromColumn,
        },
      },
  ...
  private async _assertUploadSizeBelowLimit(upload: UploadInfo) {
    ...
    if (await this._isUploadSizeBelowLimit(uploadSizeBytes)) {
      return;
    }
   ...
   */
  private async _isUploadSizeBelowLimit(
...
export function colIdToRef(
  ...
  const [, , colRefs, columnData] = metaTables._grist_Tables_column;
  const colRowIndex = columnData.colId.findIndex(
    (_, i) =>
